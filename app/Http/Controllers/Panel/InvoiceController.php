...
//        $this->send_notif_to_sales_manager($invoice);
    ...
    public function edit(Invoice $invoice)
    {
        ...
        // Additional checks for sales-manager role
        if (Gate::allows('sales-manager')) {
            if ($invoice->created_in === 'website' && $invoice->req_for !== 'amani-invoice') {
                return back()->withErrors(['error' => 'You are not allowed to edit this invoice.']);
            }
        } else {
            if (
                $invoice->created_in === 'website' ||
                ($invoice->status === 'invoiced' && $invoice->req_for !== 'amani-invoice')
            ) {
                return back()->withErrors(['error' => 'You are not allowed to edit this invoice.']);
            }
        }
        ...
        if (!$order) {
            return back()->withErrors(['error' => 'Related order not found.']);
        }
