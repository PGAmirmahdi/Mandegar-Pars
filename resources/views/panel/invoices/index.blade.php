                ...
                <table class="table table-striped table-bordered dataTable dtr-inline text-center dataTable">
                    ...
                    @foreach($invoices as $key => $invoice)
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge badge-primary d-block">{{ \App\Models\Invoice::STATUS[$invoice->status] }}</span>
                            </td>
                            {{--                            @canany(['accountant', 'sales-manager'])--}}
                            ...
                            <td>
                                ...
                                <a href="{{ route('orders-status.index', $invoice->id) }}"
                                   class="btn btn-gradient-warning btn-floating {{ $invoice->created_at < verta('2024-02-03 00:00:00') ? 'disabled' : '' }}"
                                   target="_blank">
                                    ...
                                    @can('invoices-delete')
                                        <td>
                                            <button class="btn btn-danger btn-floating trashRow"
                                                    ...
                                                    data-id="{{ $invoice->id }}" {{ $invoice->created_in == 'website' ? 'disabled' : '' }} @canany(['admin','warehouse','accountant','ceo']) @else
                                                {{ $invoice->status == 'pending' ? 'disabled' : '' }}
                                                @endcanany>
                                ...
                                @else
                                    ...
                                    @can('invoices-delete')
                                        <td>
                                            <button class="btn btn-danger btn-floating trashRow"
                                                    data-url="{{ route('invoices.destroy',$invoice->id) }}"
                                                    data-id="{{ $invoice->id }}" {{ $invoice->created_in == 'website' || $invoice->status == 'invoiced' ? 'disabled' : '' }} @canany(['admin','warehouse','accountant','ceo']) @else
                                                {{ $invoice->status == 'pending' ? 'disabled' : '' }}
                                                @endcanany>
                                                <i class="fa fa-trash"></i>
                                            </button>
