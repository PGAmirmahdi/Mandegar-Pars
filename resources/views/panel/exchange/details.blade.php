            ...
            <form method="GET" action="{{ route('exchange.details', ['item' => $item]) }}">
                <div class="row">
                    <div class="col-xl-2 col-md-3">
                        <input type="text" class="form-control date-picker-shamsi-list" id="start_date"
                               placeholder="تاریخ شروع"
                               name="start_date"
                               value="">
                    </div>
                    <div class="col-xl-2 col-md-3">
                        <input type="text" class="form-control date-picker-shamsi-list" id="end_date"
                        ...
                        <tr>
                            ...
                            @if(number_format($entry['close'] - $entry['open']) > 0)
                                <td class="text-success">
                                    <i class="fa fa-up-long text-success mr-2"></i> {{ number_format($entry['close'] - $entry['open']) }}
                                    + تومان
                                </td>
                            @elseif(number_format($entry['close'] - $entry['open']) < 0)
                                <td class="text-danger">
                                    <i class="fa fa-down-long text-danger mr-2"></i> {{ number_format($entry['close'] - $entry['open']) }}
                                    تومان
                                </td>
                            @else
